import { initidStruct } from "../../Common/initStruct/all_init_Fn";
...
const sendAttatchMsgNativeFunction = (contactId: any, path: any): void => {
  ...
  Memory.patchCode(attatchAsm, Process.pageSize, (code) => {
    ...
    cw.putPushReg("eax");
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset1));
    ...
    cw.putPushU32(-1);
    cw.putPushU32(
      moduleBaseAddress.add(offset.send_attatch_call_para).toInt32()
    );
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset2));

    cw.putSubRegImm("esp", 0x14);
    cw.putMovRegReg("ecx", "esp");
    cw.putPushU32(attatchPath.toInt32());
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset3));

    cw.putSubRegImm("esp", 0x14);
    cw.putMovRegReg("ecx", "esp");
    cw.putPushU32(attatchWxid.toInt32());
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset4));

    cw.putMovRegAddress("eax", attatchBuf);
    cw.putPushReg("eax");
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset5));

    cw.putMovRegReg("ecx", "eax");
    cw.putCallAddress(moduleBaseAddress.add(offset.send_attatch_call_offset6));
